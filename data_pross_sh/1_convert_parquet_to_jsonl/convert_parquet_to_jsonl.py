...
def convert_numpy_types(obj):
    ...
    if isinstance(obj, np.integer):
        ...
        # Check for NaN/None values, but handle potential errors
        try:
            if pd.isna(obj):
                return None
        except (TypeError, KeyError, ValueError):
            # If pd.isna() fails, just return the object as is
            pass
        return obj
...
def find_parquet_files(base_dir, pattern="*.parquet"):
    ...
    for parquet_file in base_path.rglob(pattern):
        ...
        if  ("/meta/" in file_str or "\\meta\\" in file_str):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--base_dir",
        type=str,
        default="/mnt/dolphinfs/ssd_pool/docker/user/hadoop-nlp-sh02/native_mm/yiyang11/datasets/lerobot",
