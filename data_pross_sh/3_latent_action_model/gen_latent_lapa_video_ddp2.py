...
def load_metadata_from_jsonl(metadata_file):
...
def save_metadata_to_jsonl(metadata_file, metadata_item):
...
def scan_video_metadata(video_files, model_type, base_output_dir, rank=0):
    ...
    # Define metadata jsonl file path
    metadata_file = os.path.join(base_output_dir, model_type, "video_metadata.jsonl")
    os.makedirs(os.path.dirname(metadata_file), exist_ok=True)

    # Load existing metadata
    existing_metadata = load_metadata_from_jsonl(metadata_file)
    if rank == 0:
        print(f"Loaded {len(existing_metadata)} existing metadata records from {metadata_file}")
    ...
    for i, vf in enumerate(video_files):
        ...
        # Check if metadata already exists in jsonl
        if vf in existing_metadata:
            metadata.append(existing_metadata[vf])
        ...
        try:
            ...
            if rank == 0:
                save_metadata_to_jsonl(metadata_file, metadata_item)
                new_count += 1
...
if __name__ == "__main__":
    ...
    parser.add_argument("--slice_size", type=int, default=10, help="Number of frames per slice for distributed processing")
