...
class BridgeDataset:
    ...
    def extracdeo(self, output_path: str = "video_0.mp4", fps: int = 30, num_frames: int = None):
        """
        Extract video from dataset observations.

        Args:
            output_path: Path to save the video file (default: "video_0.mp4")
            fps: Frames per second for the output video (default: 30)
            num_frames: Number of frames to extract. If None, extract all frames from first trajectory.
        ...
        try:
            for batch in iterator:
                # Extract observation images from batch
                obs_images = batch["observations"]["image"]

                # Process each frame in the batch
                for frame_idx in range(obs_images.shape[0]):
                    frame = obs_images[frame_idx]
        ...
        if frames:
            ...
            out.release()
            logging.info(f"Video saved to {output_path} with {len(frames)} frames at {fps} fps")
        else:
            logging.warning("No frames extracted from dataset")
