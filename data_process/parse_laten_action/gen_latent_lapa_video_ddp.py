...
def _build_output_path(video_file, model_type, base_output_dir, is_temp=False, start_frame=None, rank=0):
    ...
    if is_temp and start_frame is not None:
        ...
        output_path = os.path.join(temp_dir, f"{video_stem}_slice_{start_frame:06d}.pt")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--clean-only", action="store_true", help="Only clean up temporary slice files, skip processing and merging")
    ...
    # If clean-only mode, only clean up temp files
    if args.clean_only:
        print("Clean-only mode: Only cleaning up temporary slice files")
        slice_files = find_slice_files(args.video_dir, model_type)
        if len(slice_files) > 0:
            cleanup_temp_slices(slice_files, keep_temp=False)
        else:
            print("No temporary slice files found to clean up.")
